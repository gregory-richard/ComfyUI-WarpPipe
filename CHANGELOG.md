...
- **Used LoRAs shown on the node**: each tag in the prompt appears as a row with its preview, a weight you can drag, buttons to reorder or remove it, and a control to insert its trigger words. The prompt text stays the single source of truth; every control rewrites the tag in place.
- **`/` in the prompt** opens a picker over both LoRAs and embeddings, inserting `<lora:...>` or `embedding:...` as appropriate.
